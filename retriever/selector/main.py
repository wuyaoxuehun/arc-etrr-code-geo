...
if __name__ == '__main__':
    ...
    model = Model(args)
    data = load_data('./data/keywordAndRelevancePassage_h-processed.json', model.elmo)
    import random
    random.seed(1234)
    random.shuffle(data)
    ...
    checkpoint_path = 'checkpoint/%d-%s.mdl' % (args.seed, datetime.now().__str__().replace(':', '-')[:-7])
    ...
    print('Best dev f1={:.3f}\nBest dev recall={:.3f}\nBest dev precison={:.3f}'.format(best_dev_f1, best_recall,best_precision))
