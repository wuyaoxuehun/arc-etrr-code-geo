...
class Example:
    ...
    def __init__(self, input_dict, elmo=None, use_science_term=True):
        self.id = input_dict['id']
        self.question = input_dict['q_words']
        self.choice = input_dict['c_words']
        self.q_pos = input_dict['q_pos']
        self.q_ner = input_dict['q_ner']
        assert len(self.q_pos) == len(self.question.split()), (self.q_pos, self.question,len(self.q_pos), len(self.question.split()))

        self.features = np.stack([input_dict['in_c'], input_dict['tf'], input_dict['q_is_science_term']], 1)
